...
import { panelStateKey } from "./interaction-store.js";
import { getPanelExpandToken, getPanelSpec, subscribePanel, subscribePanelExpand } from "./panel-store.js";
 ...
 */
export function GenuiPanel({ sessionId, sendGenuiAction }) {
    ...
    if (spec === null || spec.items.length === 0)
        return null;
    return (_jsxs("div", { className: css.panel, "data-genui-panel": true, children: [!collapsed && (_jsx("div", { role: "separator", "aria-orientation": "horizontal", "aria-label": "\u8C03\u6574\u9762\u677F\u9AD8\u5EA6", className: `${css.panelResizeHandle}${resizing ? ` ${css.panelResizeHandleActive}` : ''}`, onPointerDown: startResize })), _jsx("div", { className: css.panelHeader, children: _jsxs("button", { type: "button", className: css.panelToggle, "aria-expanded": !collapsed, onClick: () => setCollapsed(c => !c), children: [_jsx("span", { className: css.panelBadge, children: "\u9762\u677F" }), _jsx("span", { className: css.panelTitle, children: spec.title ?? 'GenUI 面板' }), _jsx("span", { className: css.panelChevron, "aria-hidden": true, children: collapsed ? '▸' : '▾' })] }) }), !collapsed && (_jsx("div", { ref: bodyRef, className: css.panelBody, "data-genui-panel-body": true, style: bodyHeight === null ? undefined : { height: bodyHeight }, children: _jsx(GenuiActionContext.Provider, { value: sendGenuiAction, children: _jsx(ErrorBoundary, { label: "\u9762\u677F", children: _jsx(GenuiBlock, { spec: spec, stateKey: panelStateKey(sessionId, JSON.stringify(spec)) }) }) }) }))] }));
}

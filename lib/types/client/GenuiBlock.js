...
import { loadBlockState, saveBlockState } from "./interaction-store.js";
...
function ClickFeedbackButton({ className, disabled, onClick, children }) {
    ...
    return (_jsxs("button", { type: "button", className: className, disabled: disabled, onClick: onClick === undefined ? undefined : () => {
            onClick();
            if (timer.current !== null)
                clearTimeout(timer.current);
            setSent(true);
            timer.current = setTimeout(() => setSent(false), 1400);
        }, children: [children, sent && _jsx("span", { className: css.btnSent, children: "\u2713 \u5DF2\u54CD\u5E94" })] }));
}
function renderNode(node, key, onAction, depth = 0, answers) {
    ...
    switch (node.type) {
        ...
        case 'row': {
            return (_jsxs("div", { className: css.row + (node.wrap ? ` ${css.wrap}` : ''), children: [node.items.map((c, i) => renderNode(c, i, onAction, depth + 1, answers)), node.spacer && _jsx("div", { className: css.spacer })] }, key));
        }
        case 'col': {
            return (_jsx("div", { className: css.col, style: node.gap !== undefined ? { gap: `${node.gap}px` } : undefined, children: node.items.map((c, i) => renderNode(c, i, onAction, depth + 1, answers)) }, key));
        }
        case 'grid': {
            return (_jsx("div", { className: css.grid, style: { gridTemplateColumns: `repeat(${Math.max(1, node.cols)}, 1fr)` }, children: node.items.map((c, i) => renderNode(c, i, onAction, depth + 1, answers)) }, key));
        }
        case 'card': {
            return (_jsxs("div", { className: css.card, children: [node.title !== undefined && _jsx("div", { className: css.cardTitle, children: node.title }), node.items.map((c, i) => renderNode(c, i, onAction, depth + 1, answers))] }, key));
        }
        case 'button': {
            ...
            return (_jsxs(ClickFeedbackButton, { className: cls, disabled: !interactive, onClick: interactive ? () => onAction(action, { type: 'button', label: node.label }) : undefined, children: [node.icon !== undefined && _jsxs("span", { "aria-hidden": true, children: [node.icon, " "] }), node.label] }, key));
        }
        case 'input': return _jsx(InputNode, { node: node, onAction: onAction, answers: answers }, key);
        case 'select': {
            const action = node.action;
        ...
        case 'radio': return _jsx(RadioNode, { node: node, onAction: onAction, answers: answers }, key);
        case 'submit': return _jsx(SubmitNode, { node: node, onAction: onAction, answers: answers }, key);
        case 'switch': return _jsx(SwitchNode, { node: node, onAction: onAction }, key);
        case 'textarea': return _jsx(TextareaNode, { node: node, onAction: onAction, answers: answers }, key);
        case 'accordion': return _jsx(AccordionNode, { node: node, onAction: onAction, depth: depth + 1, answers: answers }, key);
        ...
        case 'quiz': return _jsx(QuizNode, { node: node, onAction: onAction }, key);
        default: {
            ...
            if (Custom !== undefined) {
                return (_jsx(Custom, { node: custom, onAction: onAction, renderChildren: (nodes, base) => nodes.map((c, i) => renderNode(c, Number(base) + i, onAction, depth + 1, answers)) }, key));
            }
            return null;
        }
    }
...
function TabsNode({ tabs, onAction, depth = 0, answers }) {
    ...
    return (_jsxs("div", { className: css.tabs, children: [_jsx("div", { className: css.tabBar, role: "tablist", "aria-orientation": "horizontal", onKeyDown: e => {
                    if (e.key === 'ArrowRight') {
                        e.preventDefault();
                        move(active + 1);
                    }
...
function RadioNode({ node, onAction, answers }) {
    ...
    const restoredIndex = group !== undefined && answers?.answers[group] !== undefined
        ...
        : -1;
    const [selected, setSelected] = useState(restoredIndex >= 0 ? restoredIndex : (node.selected ?? null));
    const uid = useId();
    ...
    useEffect(() => {
        if (answers?.round === mountedRound.current)
            return;
    ...
    return (_jsxs("div", { className: css.fieldGroup, role: "radiogroup", "aria-label": node.label, children: [node.label !== undefined && _jsx("span", { className: css.fieldLabel, children: node.label }), options.map((opt, i) => (_jsxs("label", { className: css.radio, children: [_jsx("input", { type: "radio", name: `genui-radio-${uid}`, checked: selected === i, disabled: locked, onChange: () => {
                            setSelected(i);
                            if (grouped) {
...
function correctLabelOf(m) {
    if (m.answer === undefined)
        return undefined;
    if (typeof m.answer === 'number')
        return m.options[m.answer];
    return m.answer;
}
...
function SubmitNode({ node, onAction, answers }) {
    ...
    if (submitted) {
        ...
        return (_jsxs("div", { className: css.gradeWrap, "data-genui-grade": true, children: [_jsxs("div", { className: css.gradeScore, children: [_jsxs("span", { className: css.gradeScoreValue, children: [score, " / ", graded.length] }), _jsxs("span", { className: css.gradeScoreLabel, children: ["\u5F97\u5206", graded.length < scope.length ? `（${scope.length - graded.length} 题无答案未计分）` : ''] })] }), _jsx("div", { className: css.gradeList, children: scope.map(g => {
                        const entry = recorded[g];
                        const m = meta[g];
                        if (entry === undefined || m === undefined)
                            return null;
                        const correct = correctLabelOf(m);
                        if (correct === undefined) {
                            return (_jsxs("div", { className: css.gradeItem, children: [_jsx("span", { className: css.gradeQ, children: m.label }), _jsxs("span", { className: css.gradeAns, children: ["\u4F60\u7684\u7B54\u6848\uFF1A", entry.choice] })] }, g));
                        }
                        const isCorrect = entry.choice === correct;
                        return (_jsxs("div", { className: `${css.gradeItem} ${isCorrect ? css.gradeItemOk : css.gradeItemNo}`, children: [_jsx("span", { className: css.gradeQ, children: m.label }), _jsx("span", { className: css.gradeTag, children: isCorrect ? '✓' : '✗' }), _jsxs("span", { className: css.gradeAns, children: ["\u4F60\u7684\u7B54\u6848\uFF1A", entry.choice, !isCorrect && _jsxs("span", { className: css.gradeRight, children: [" \u6B63\u786E\u7B54\u6848\uFF1A", correct] })] }), m.explanation !== undefined && _jsx("span", { className: css.gradeExp, children: m.explanation })] }, g));
                    }) }), _jsx("button", { type: "button", className: `${css.button} ${css.ghost} ${css.submit}`, onClick: () => {
                        answers?.clear();
    ...
    return (_jsxs("div", { className: css.submitRow, children: [_jsx("button", { type: "button", className: `${css.button} ${css.primary} ${css.submit}`, disabled: !ready, onClick: ready ? () => {
                    if (canGradeLocally) {
                ...
                } : undefined, children: node.label }), total > 0 && _jsxs("span", { className: css.submitHint, "aria-live": "polite", children: ["\u5DF2\u9009 ", answered, "/", total] })] }));
}
/** Switch: toggle with local state. */
function SwitchNode({ node, onAction }) {
    const [on, setOn] = useState(node.checked === true);
    const action = node.action;
    return (_jsxs("label", { className: css.switchRow, children: [_jsx("span", { className: css.switchLabel, children: node.label }), _jsx("button", { type: "button", role: "switch", "aria-checked": on, className: `${css.switch} ${on ? css.switchOn : ''}`, onClick: () => {
                    const next = !on;
                    setOn(next);
...
function InputNode({ node, onAction, answers }) {
    ...
    const [value, setValue] = useState(() => node.value ?? (id !== undefined ? answers?.fields[id] ?? '' : ''));
    const send = (submit) => {
        if (action !== undefined && onAction !== undefined) {
            onAction(action, { type: 'input', value, ...(submit ? { submit: true } : {}) });
        }
    };
    return (_jsxs("label", { className: css.field, children: [node.label !== undefined && _jsx("span", { children: node.label }), _jsx("input", { className: css.input, type: node.inputType ?? 'text', placeholder: node.placeholder, value: value, onChange: e => {
                    const v = e.currentTarget.value;
                    setValue(v);
                    if (id !== undefined)
                        answers?.setField(id, v);
                }, onBlur: () => send(false), onKeyDown: e => {
                    if (e.key === 'Enter') {
                        e.preventDefault();
                        send(true);
                    }
...
function TextareaNode({ node, onAction, answers }) {
    ...
    return (_jsxs("label", { className: css.field, children: [node.label !== undefined && _jsx("span", { children: node.label }), _jsx("textarea", { className: css.textarea, placeholder: node.placeholder, rows: node.rows ?? 4, value: value, onChange: e => {
                    const v = e.currentTarget.value;
                    setValue(v);
                    if (id !== undefined)
                        answers?.setField(id, v);
...
function MermaidNode({ node }) {
    ...
    if (failed)
        return _jsxs("div", { className: css.mermaidFallback, children: [_jsx("pre", { children: code }), _jsx("div", { className: css.mermaidErr, children: "\u56FE\u8BED\u6CD5\u6709\u8BEF\uFF0C\u5DF2\u964D\u7EA7\u663E\u793A\u6E90\u7801" })] });
    if (html === null)
...
function QuizNode({ node, onAction }) {
    ...
    const action = node.action;
    return (_jsxs("div", { className: css.quiz, "data-genui-quiz": true, children: [_jsx("div", { className: css.quizQuestion, children: node.question }), _jsx("div", { className: css.quizOptions, children: options.map((opt, i) => {
                    ...
                    const cls = answered
                        ...
                        : css.quizOpt;
                    return (_jsxs("button", { type: "button", className: cls, disabled: answered, onClick: () => {
                            setSelected(i);
                            if (action !== undefined && onAction !== undefined) {
                                onAction(action, {
                                    type: 'quiz',
                        ...
                        }, children: [_jsx("span", { className: css.quizMarker, children: answered && (opt.correct === true ? '✓' : isChosen ? '✗' : '') }), opt.label] }, i));
 ...
 */
export const GenuiBlock = memo(function GenuiBlock({ spec, stateKey }) {
    ...
    const setAnswer = useCallback((group, entry) => {
        setAnswers(prev => {
    ...
    const registerMeta = useCallback((group, m) => {
        setMeta(prev => {
            const existing = prev[group];
    ...
    const answersState = useMemo(() => ({ answers, fields, meta, locked, round, setAnswer, setField, registerMeta, clear, setLocked }), [answers, fields, meta, locked, round, setAnswer, setField, registerMeta, clear]);
    ...
    return (_jsxs("div", { className: css.block, "data-genui": true, children: [spec.title !== undefined && _jsx("div", { className: css.banner, children: spec.title }), _jsx("div", { className: css.col, style: { gap: `${gap}px` }, children: spec.items.map((c, i) => (
                ...
                _jsx("div", { className: css.reveal, style: { animationDelay: `${Math.min(i * 90, 720)}ms` }, children: renderNode(c, i, onAction, 0, answersState) }, i))) })] }));
});

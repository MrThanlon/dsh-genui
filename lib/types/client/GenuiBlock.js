...
function renderNode(node, key, onAction) {
    switch (node.type) {
        case 'text': {
        ...
        case 'row': {
            return (_jsxs("div", { className: css.row + (node.wrap ? ` ${css.wrap}` : ''), children: [node.items.map((c, i) => renderNode(c, i, onAction)), node.spacer && _jsx("div", { className: css.spacer })] }, key));
        }
        case 'col': {
            return (_jsx("div", { className: css.col, style: node.gap !== undefined ? { gap: `${node.gap}px` } : undefined, children: node.items.map((c, i) => renderNode(c, i, onAction)) }, key));
        }
        case 'grid': {
            return (_jsx("div", { className: css.grid, style: { gridTemplateColumns: `repeat(${Math.max(1, node.cols)}, 1fr)` }, children: node.items.map((c, i) => renderNode(c, i, onAction)) }, key));
        }
        case 'card': {
            return (_jsxs("div", { className: css.card, children: [node.title !== undefined && _jsx("div", { className: css.cardTitle, children: node.title }), node.items.map((c, i) => renderNode(c, i, onAction))] }, key));
        }
        case 'button': {
        ...
        case 'select': {
            ...
            return (_jsxs("label", { className: css.field, children: [node.label !== undefined && _jsx("span", { children: node.label }), _jsx("select", { className: css.select, onChange: action !== undefined && onAction !== undefined
                            ...
                            : undefined, children: node.options.map((o, i) => _jsx("option", { children: o }, i)) })] }, key));
        }
        case 'checkbox': {
            const action = node.action;
        ...
        case 'progress': {
            ...
            return (_jsxs("div", { className: css.progress, children: [(node.label !== undefined || node.valueLabel !== undefined) && (_jsxs("div", { className: css.progressRow, children: [_jsx("span", { children: node.label }), node.valueLabel !== undefined && _jsx("span", { children: node.valueLabel })] })), _jsx("div", { className: css.track, children: _jsx("div", { className: css.fill, style: { width: `${v}%` } }) })] }, key));
        }
        case 'divider': return _jsx("hr", { className: css.divider }, key);
        case 'list': {
            return (_jsx("div", { className: css.list, children: node.items.map((item, i) => (_jsx("div", { className: css.li, children: typeof item === 'string'
        ...
        case 'table': {
            return (_jsx("div", { className: css.tableWrap, children: _jsxs("table", { className: css.table, children: [_jsx("thead", { children: _jsx("tr", { children: node.columns.map((c, i) => _jsx("th", { children: c }, i)) }) }), _jsx("tbody", { children: node.rows.map((row, i) => (_jsx("tr", { children: row.map((cell, j) => _jsx("td", { children: String(cell) }, j)) }, i))) })] }) }, key));
        }
        case 'chart': return _jsx(ChartNode, { chart: node }, key);
        case 'tabs': return _jsx(TabsNode, { tabs: node, onAction: onAction }, key);
        case 'avatar': {
        ...
        case 'accordion': return _jsx(AccordionNode, { node: node, onAction: onAction }, key);
        ...
        default: {
            ...
            if (Custom !== undefined) {
                return (_jsx(Custom, { node: custom, onAction: onAction, renderChildren: (nodes, base) => nodes.map((c, i) => renderNode(c, Number(base) + i, onAction)) }, key));
            }
            return null;
        }
    }
...
function StepsNode({ steps }) {
    ...
    return (_jsx("ol", { className: css.steps, children: steps.steps.map((step, i) => {
...
function KeyValueNode({ node }) {
    return (_jsx("dl", { className: css.keyvalue, children: node.pairs.map((pair, i) => (_jsxs("div", { className: css.kvRow, children: [_jsx("dt", { className: css.kvKey, children: pair.key }), _jsx("dd", { className: css.kvValue, children: pair.value })] }, i))) }));
}
/** Plot: SVG function plot over the SafeMath evaluator. */
function PlotNode({ plot }) {
    return (_jsx(PlotBlock, { series: plot.series.map(s => ({ expr: s.expr, label: s.label, color: s.color, params: s.params })), xMin: plot.xMin, xMax: plot.xMax, yMin: plot.yMin, yMax: plot.yMax, title: plot.title }));
}
...
function CodeNode({ node }) {
    return _jsx(CodeBlock, { code: node.code, lang: node.lang });
}
...
function TabsNode({ tabs, onAction }) {
    const [active, setActive] = useState(0);
    const current = tabs.tabs[active];
    return (_jsxs("div", { className: css.tabs, children: [_jsx("div", { className: css.tabBar, role: "tablist", children: tabs.tabs.map((tab, i) => (_jsx("button", { type: "button", role: "tab", "aria-selected": i === active, className: `${css.tab} ${i === active ? css.tabActive : ''}`, onClick: () => setActive(i), children: tab.label }, i))) }), current !== undefined && (_jsx("div", { className: css.col, role: "tabpanel", children: current.items.map((c, i) => renderNode(c, i, onAction)) }))] }));
}
...
function RadioNode({ node, onAction }) {
    ...
    const action = node.action;
    return (_jsxs("div", { className: css.fieldGroup, role: "radiogroup", "aria-label": node.label, children: [node.label !== undefined && _jsx("span", { className: css.fieldLabel, children: node.label }), node.options.map((opt, i) => (_jsxs("label", { className: css.radio, children: [_jsx("input", { type: "radio", name: `genui-radio-${Math.random().toString(36).slice(2)}`, checked: i === selected, onChange: () => {
                            setSelected(i);
...
function AccordionNode({ node, onAction }) {
    const [open, setOpen] = useState(0);
    return (_jsx("div", { className: css.accordion, children: node.items.map((item, i) => (_jsxs("div", { className: css.accItem, children: [_jsxs("button", { type: "button", className: css.accHead, "aria-expanded": open === i, onClick: () => setOpen(open === i ? null : i), children: [_jsx("span", { className: css.accTitle, children: item.title }), _jsx("span", { className: css.accChevron, children: open === i ? '▾' : '▸' })] }), open === i && (_jsx("div", { className: css.accBody, children: item.items.map((c, ci) => renderNode(c, ci, onAction)) }))] }, i))) }));
}
...
function MermaidNode({ node }) {
    ...
    if (failed)
        return _jsxs("div", { className: css.mermaidFallback, children: [_jsx("pre", { children: node.code }), _jsx("div", { className: css.mermaidErr, children: "mermaid \u6E32\u67D3\u5931\u8D25" })] });
    if (html === null)
        return _jsxs("div", { className: css.mermaidFallback, children: [_jsx("pre", { children: node.code }), _jsx("div", { className: css.mermaidHint, children: "\u6E32\u67D3\u4E2D\u2026" })] });
...
function TimelineNode({ node }) {
    return (_jsx("div", { className: css.timeline, children: node.items.map((item, i) => (_jsxs("div", { className: css.tlItem, children: [_jsxs("div", { className: css.tlRail, children: [_jsx("span", { className: css.tlDot }), i < node.items.length - 1 && _jsx("span", { className: css.tlLine })] }), _jsxs("div", { className: css.tlBody, children: [_jsxs("div", { className: css.tlHead, children: [_jsx("span", { className: css.tlTitle, children: item.title }), item.time !== undefined && _jsx("span", { className: css.tlTime, children: item.time })] }), item.desc !== undefined && _jsx("div", { className: css.tlDesc, children: item.desc })] })] }, i))) }));
}
...
function FileTreeNode({ node }) {
    ...
    return _jsx("div", { className: css.fileTree, children: node.items.map((n, i) => renderNode(n, 0, i)) });
}
...
function QuizNode({ node }) {
    ...
    return (_jsxs("div", { className: css.quiz, "data-genui-quiz": true, children: [_jsx("div", { className: css.quizQuestion, children: node.question }), _jsx("div", { className: css.quizOptions, children: node.options.map((opt, i) => {
 ...
 */
export const GenuiBlock = memo(function GenuiBlock({ spec }) {
    ...
    return (_jsxs("div", { className: css.block, "data-genui": true, children: [spec.title !== undefined && _jsx("div", { className: css.banner, children: spec.title }), _jsx("div", { className: css.col, style: { gap: `${gap}px` }, children: spec.items.map((c, i) => (
                ...
                _jsx("div", { className: css.reveal, style: { animationDelay: `${Math.min(i * 90, 720)}ms` }, children: renderNode(c, i, onAction) }, i))) })] }));
});

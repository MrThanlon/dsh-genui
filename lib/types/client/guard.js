...
const TEXT_SIZES = ['h1', 'h2', 'h3', 'body', 'muted', 'caption'];
const BUTTON_TONES = ['primary', 'danger', 'success', 'ghost'];
const BADGE_TONES = ['success', 'warn', 'danger', 'accent'];
const INPUT_TYPES = ['text', 'email', 'password'];
const CALLOUT_TONES = ['info', 'success', 'warning', 'error'];
const CHART_KINDS = ['bars', 'line', 'donut'];
const MESH_SHAPES = ['box', 'sphere', 'cone', 'cylinder', 'torus'];
...
function repairNode(value, ctx, depth) {
    ...
    switch (type) {
        ...
        case 'chart': {
            ...
            if (data === undefined && series === undefined)
                return null;
...
function repairRows(v, rowCap, colCap) {
    ...
    for (const row of v) {
        ...
        for (const cell of row) {
            if (cells.length >= colCap)
                break;
            if (typeof cell === 'string')
                cells.push(cell.slice(0, 256));
...
function repairTabs(v, ctx, depth) {
    ...
    const out = [];
    for (const tab of v) {
        if (out.length >= GENUI_LIMITS.maxTabs)
            break;
        const o = obj(tab);
        const label = o === undefined ? undefined : str(o.label, 128);
        if (label === undefined || o === undefined)
            continue;
...
function repairPlotSeries(v, cap) {
    ...
    for (const s of v) {
        ...
        if (expr === undefined || o === undefined)
            continue;
        const params = [];
        if (Array.isArray(o.params)) {
            for (const p of o.params) {
                if (params.length >= GENUI_LIMITS.maxPlotParams)
                    break;
                const po = obj(p);
...
function repairSteps(v) {
    ...
    const out = [];
    for (const s of v) {
        if (out.length >= GENUI_LIMITS.maxSteps)
            break;
        const o = obj(s);
...
function repairAccordion(v, ctx, depth) {
    ...
    const out = [];
    for (const item of v) {
        if (out.length >= GENUI_LIMITS.maxAccordionItems)
            break;
        const o = obj(item);
        const title = o === undefined ? undefined : str(o.title, 256);
        if (title === undefined || o === undefined)
            continue;
...
function repairMeshes(v) {
    ...
    const out = [];
    for (const m of v) {
        if (out.length >= GENUI_LIMITS.maxMeshes)
            break;
        const o = obj(m);
...
function repairQuizOptions(v) {
    ...
    for (const optItem of v) {
        if (out.length >= GENUI_LIMITS.maxQuizOptions)
            break;
        const o = obj(optItem);
 ...
 */
export function repairGenuiSpec(value) {
    const v = obj(value);
    if (v === undefined || !Array.isArray(v.items))
        return null;
 ...
 */
export function validateGenuiSpec(value) {
    const errors = [];
    const v = obj(value);
    if (v === undefined)
    ...
    if (v.gap !== undefined && (typeof v.gap !== 'number' || !Number.isFinite(v.gap)))
        errors.push('spec.gap must be a finite number');
    let count = 0;
    let capped = false;
...
function validateNode(value, depth, at, errors, walk) {
    ...
    const isStr = (name) => { if (v[name] !== undefined && typeof v[name] !== 'string')
        errors.push(`${at}: '${name}' must be a string`); };
    const isNum = (name) => { if (v[name] !== undefined && (typeof v[name] !== 'number' || !Number.isFinite(v[name])))
        errors.push(`${at}: '${name}' must be a finite number`); };
    switch (type) {
        case 'text':
            if (typeof v.content !== 'string')
                errors.push(`${at}: type 'text' requires content (string)`);
            isStr('content');
            break;
        case 'row':
        case 'col':
        ...
        case 'switch':
            if (typeof v.label !== 'string')
                errors.push(`${at}: type '${type}' requires label (string)`);
            isStr('label');
            break;
        case 'input':
        case 'textarea':
        ...
        case 'radio':
            if (!Array.isArray(v.options))
                errors.push(`${at}: type '${type}' requires options (array)`);
            break;
        case 'submit':
            if (typeof v.label !== 'string')
                errors.push(`${at}: type 'submit' requires label (string)`);
            if (typeof v.action !== 'string')
                errors.push(`${at}: type 'submit' requires action (string)`);
            break;
        case 'badge':
            if (typeof v.label !== 'string')
                errors.push(`${at}: type 'badge' requires label (string)`);
            isStr('label');
            break;
        case 'stat':
            if (typeof v.label !== 'string')
                errors.push(`${at}: type 'stat' requires label (string)`);
            if (typeof v.value !== 'string')
                errors.push(`${at}: type 'stat' requires value (string)`);
            isStr('delta');
            break;
        case 'progress':
            if (typeof v.value !== 'number' || !Number.isFinite(v.value) || v.value < 0 || v.value > 100) {
        ...
        case 'avatar':
            if (typeof v.name !== 'string')
                errors.push(`${at}: type 'avatar' requires name (string)`);
            break;
        case 'list':
            if (!Array.isArray(v.items))
                errors.push(`${at}: type 'list' requires items (array)`);
            break;
        case 'table':
            if (!Array.isArray(v.columns))
                errors.push(`${at}: type 'table' requires columns (array)`);
            if (!Array.isArray(v.rows))
                errors.push(`${at}: type 'table' requires rows (array)`);
            break;
        case 'chart':
        ...
        case 'tabs': {
            if (!Array.isArray(v.tabs))
                errors.push(`${at}: type 'tabs' requires tabs (array)`);
            if (Array.isArray(v.tabs)) {
                for (let i = 0; i < v.tabs.length; i++) {
                    ...
                    if (typeof t.label !== 'string')
                        errors.push(`${at}.tabs[${i}].label must be a string`);
        ...
        case 'plot':
            if (!Array.isArray(v.series))
                errors.push(`${at}: type 'plot' requires series (array)`);
            break;
        case 'callout':
            if (typeof v.content !== 'string')
                errors.push(`${at}: type 'callout' requires content (string)`);
            break;
        case 'steps':
            if (!Array.isArray(v.steps))
                errors.push(`${at}: type 'steps' requires steps (array)`);
            break;
        case 'keyvalue':
            if (!Array.isArray(v.pairs))
                errors.push(`${at}: type 'keyvalue' requires pairs (array)`);
            break;
        case 'diff':
        ...
        case 'json':
            if (!('value' in v))
                errors.push(`${at}: type 'json' requires value`);
            break;
        case 'code':
            if (typeof v.code !== 'string')
                errors.push(`${at}: type 'code' requires code (string)`);
            break;
        case 'accordion':
            if (!Array.isArray(v.items))
                errors.push(`${at}: type 'accordion' requires items (array)`);
            if (Array.isArray(v.items)) {
                for (let i = 0; i < v.items.length; i++) {
                    ...
                    if (typeof item.title !== 'string')
                        errors.push(`${at}.items[${i}].title must be a string`);
        ...
        case 'copy':
            if (typeof v.text !== 'string')
                errors.push(`${at}: type 'copy' requires text (string)`);
            break;
        case 'mermaid':
            if (typeof v.code !== 'string')
                errors.push(`${at}: type 'mermaid' requires code (string)`);
            break;
        case 'scene3d':
            if (!Array.isArray(v.meshes))
                errors.push(`${at}: type 'scene3d' requires meshes (array)`);
            break;
        case 'timeline':
            if (!Array.isArray(v.items))
                errors.push(`${at}: type 'timeline' requires items (array)`);
            break;
        case 'file-tree':
            if (!Array.isArray(v.items))
                errors.push(`${at}: type 'file-tree' requires items (array)`);
            break;
        case 'breadcrumb':
            if (!Array.isArray(v.items))
                errors.push(`${at}: type 'breadcrumb' requires items (array)`);
            break;
        case 'quiz':
            if (typeof v.question !== 'string')
                errors.push(`${at}: type 'quiz' requires question (string)`);
            if (!Array.isArray(v.options))
                errors.push(`${at}: type 'quiz' requires options (array)`);
            break;
        default:

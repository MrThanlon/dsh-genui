...
function readStore() {
    try {
        ...
        return { order: parsed.order.filter(k => typeof k === 'string'), blocks: parsed.blocks };
    }
    catch {
        return emptyStore();
    }
...
export function loadBlockState(stateKey) {
    if (stateKey === '')
        return null;
...
export function saveBlockState(stateKey, state) {
    if (stateKey === '')
        return;
    const store = readStore();
    ...
    while (order.length > MAX_BLOCKS) {
        ...
        if (evicted !== undefined)
            delete blocks[evicted];
    }
    writeStore({ order, blocks });
}
/** Forget a block's durable state (e.g. after a reset-to-empty). */
export function clearBlockState(stateKey) {
    if (stateKey === '')
        return;
    const store = readStore();
    if (!(stateKey in store.blocks))
        return;

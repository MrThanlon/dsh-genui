 ...
 */
import { GENUI_LIMITS, repairGenuiSpec } from "../client/guard.js";
/**
...
const RENDER_UI_PARAMETERS = {
    ...
    properties: {
        spec: {
            description: 'GenUI component tree (white-listed vocabulary, see the dsh-ui fence section in the system prompt). Deep-validated and repaired by the renderer.',
        },
    },
    required: ['spec'],
...
const RENDER_UI_OUTPUT_SCHEMA = { type: 'string', description: 'One-line human-readable render summary for the model.' };
/** Read the `spec` argument defensively (presenters run on replayed args). */
function specOf(args) {
    return typeof args === 'object' && args !== null ? args.spec : undefined;
}
...
export function createRenderUiTool() {
    return {
        name: 'render_ui',
        description: 'Render an interactive UI card in the conversation tool row by passing a GenUI spec (a white-listed component tree; the same vocabulary as the ```dsh-ui fence, see the system prompt). '

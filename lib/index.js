	...
	return {
		...
		async execute(args) {
			...
			try {
				...
				return `❌ dsh-ui 围栏 JSON 解析失败：${error instanceof Error ? error.message : String(error)}。\n${bracketDiagnostic(raw)}  请按错误信息修正后重新调用本工具验证，通过后再发出围栏。\n${COMMON_CAUSES}`;
			}
...
async function serveGenuiAsset(req, res) {
	...
	const rel = pathname.startsWith(`${ASSET_ROUTE_PATH}/`) ? pathname.slice(38) : null;
	if (rel === null) {
		res.writeHead(404);
		res.end();
		return;
	...
	try {
		const body = await readFile(join(fileURLToPath(new URL("./assets/", import.meta.url)), file));
		res.writeHead(200, {
...
function apply(ctx) {
	...
	const tryRegisterAssets = (value) => {
		...
		const httpServer = value ?? ctx.reflect.get("httpServer", false);
	...
	ctx.on("internal/service", (name, value) => {
		if (name === "httpServer") tryRegisterAssets(value);
	});
}
//#endregion

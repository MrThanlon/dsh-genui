...
3. **JSON 必须严格合法**：非法围栏会退化为代码块并显示诊断（常见：缺 `}`/`]`、多了逗号、字符串没闭合）。**最容易犯的错：字符串值里用了半角引号 `"`**——中文引语一律写 `“”` 或 `「」`（插件会自动修复一部分半角引号错误，但别依赖）。发围栏前自检 3 条：① 括号配对：`{`=`}`、`[`=`]` ② 无尾随逗号 ③ 值内引号用中文引号。不要在 JSON 字符串里放 markdown；超长表格/列表拆成多个组件分开发，宁短勿长

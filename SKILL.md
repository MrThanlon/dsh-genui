...
3. **JSON 必须严格合法**：非法围栏会显示「⚠️ dsh-ui fence JSON 解析失败（含位置）」诊断条并退化为纯代码块——看到该提示说明围栏 JSON 不合法（常见：缺 `}`/`]`、多了逗号、字符串没闭合），修复后重发即可。不要在 JSON 字符串里放 markdown。**发围栏前先自检**（长 fence 尤其要做）：① 没有尾随逗号（`},` `],` 后面必须还有内容）② 每个 `{` 都有配对的 `}`、每个 `[` 都有配对的 `]` ③ 字符串里没有未转义的 `"` ④ 整个 spec 以 `}` 结尾 ⑤ 超长表格/列表拆成多个组件分开发——JSON 越长越容易漏括号，宁短勿长 ⑥ 写完数一遍：`{` 数量 = `}` 数量，`[` 数量 = `]` 数量

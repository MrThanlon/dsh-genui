...
function makeEnv(): Env {
  ...
  const pkg = join(profile, 'node_modules', '@deepseek-ai', 'dsh-genui')
  mkdirSync(pkg, { recursive: true })
  writeFileSync(join(pkg, 'package.json'), JSON.stringify({ name: '@deepseek-ai/dsh-genui', version: '0.0.0-test' }))
...
describe('install.sh skill sync safety', () => {
  ...
  it('skips a symlink that resolves to the same package file (dev ln -s case)', () => {
    const e = env()
    const pkgSkill = join(e.profile, 'node_modules', '@deepseek-ai', 'dsh-genui', 'SKILL.md')
    mkdirSync(dirname(e.dest), { recursive: true })
  ...
  it('fails loudly when the installed package lacks SKILL.md (incomplete install)', () => {
    const e = env()
    rmSync(join(e.profile, 'node_modules', '@deepseek-ai', 'dsh-genui', 'SKILL.md'))
    const { status, stdout } = e.run()
    expect(status).not.toBe(0)
